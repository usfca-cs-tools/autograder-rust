...
fn canvas_upload_mock() {
  ...
  {"student":"bob",   "score": 10, "comment": "great"}
    ...
    let sub_a = server.mock(|when, then| {
        when.method(GET)
            .path("/api/v1/courses/42/assignments/7/submissions/101");
        then.status(200)
            .header("content-type", "application/json")
    ...
    let sub_b = server.mock(|when, then| {
        when.method(GET)
            .path("/api/v1/courses/42/assignments/7/submissions/102");
        then.status(200)
            .header("content-type", "application/json")
    ...
    let canvas = CanvasCfg {
        host_name: host,
        access_token: String::from("tok"),
        course_name: String::from("Course X"),
    };
    let mapper = CanvasMapperCfg {
        map_path: csv_path.to_string_lossy().to_string(),
        github_col_name: String::from("GitHub"),
        login_col_name: String::from("SIS Login ID"),
    };
    upload_class(
        canvas,
        mapper,
...
fn canvas_pagination_and_error() {
    ...
    let _mapper = CanvasMapperCfg {
        map_path: String::from("/nonexistent.csv"),
        github_col_name: String::from("GitHub"),
        login_col_name: String::from("SIS Login ID"),
    };
