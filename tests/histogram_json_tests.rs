...
use autograder_rust::testcases::{RepoResult, TcResult, TestRunner};
...
fn histogram_and_write_json() {
    ...
    let cfg = TestCfg {
        tests_path: base.to_string_lossy().to_string(),
    ...
    let data = fs::read_to_string(base.join(&fname))
        .unwrap_or_else(|_| fs::read_to_string(fname).unwrap());
