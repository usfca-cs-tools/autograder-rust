...
"#,
    ...
    fs::write(
        base.join("tests_repo").join("dates.toml"),
        r#"
[projx]
...
fn cli_class_by_date_unified_diff_and_json() {
    ...
    write_student_repo(
        base,
        &format!("{}-{}-{}", project, "alice", suffix),
        project,
    );

    let mut cmd = Command::cargo_bin("grade-rs").unwrap();
    cmd.arg("class")
        .args(["-p", project, "-d", "-v", "--unified-diff"]) // single date auto-selects
        .env("HOME", base)
    ...
    cmd.assert()
        .success()
        .stdout(p_contains("Score frequency"))
        .stdout(p_contains("0/10"));
