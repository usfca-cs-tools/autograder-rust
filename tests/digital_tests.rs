...
fn digital_substitution_and_java_invocation() {
    ...
    let expected_line = format!(
        "-cp {} CLI test {}/1bfa_test.dig",
        digital_path.to_string_lossy(),
...
"#,
    ...
    let cfg = TestCfg {
        tests_path: tests.parent().unwrap().to_string_lossy().to_string(),
        digital_path: digital_path.to_string_lossy().to_string(),
    };
