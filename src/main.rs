...
use crate::testcases::{Repo, TestRunner};
...
fn main() {
    ...
    match command {
        ...
        Commands::Class {
            ...
            if *github_action {
                // Use GitHub API path
                let gh = match github::Github::new(
                    config.github.clone(),
                    ...
                    Ok(g) => g,
                    Err(e) => {
                        print_red(&format!("GitHub client init failed: {}\n", e));
                        std::process::exit(2);
                    }
                };
                ...
                let repos: Vec<Repo> = list
                    .iter()
                    .map(|s| Repo::student(project_name.clone(), s.clone(), None, None))
                    .collect();
                let longest = repos
                    .iter()
                    .map(|r| r.display_label.len())
                    .max()
                    .unwrap_or(0)
                    + 1;
                for s in list.iter() {
                ...
                if let Err(e) = runner.write_class_json(&class_results, None) {
                    print_red(&format!("{}\n", e));
                    std::process::exit(3);
                }
            } else {
                ...
                let (suffix_opt, _date_opt) = if *by_date {
                    ...
                    match d.select() {
                        Some(sel) => (Some(sel.suffix.clone()), Some(sel.date.clone())),
                        None => {
                            return;
                        }
                    }
                ...
                if threads == 1 {
                        ...
                        match runner_local.test_repo(r, None) {
                            Ok(rr) => {
                                ...
                                if rr.results.is_empty() {
                                    ...
                                    for t in &rr.results {
                                        let tok = crate::util::format_pass_fail(
                                            &t.test, t.rubric, t.score,
                                        );
                                        if t.score == t.rubric {
                                            crate::util::print_green(&tok);
                                        } else {
                                            crate::util::print_red(&tok);
                                        }
                                    }
                                    println!(
                                        "{}",
                                        crate::testcases::TestRunner::make_earned_avail_static(
                                            &rr.results
                                        )
                                    );
                                }
                    ...
                    let mut pending: std::collections::HashMap<String, testcases::RepoResult> =
                ...
                let mut only_results: Vec<testcases::RepoResult> =
                    class_results.into_iter().map(|(_, rr)| rr).collect();
                        ...
                        if repo_dir.is_dir() {
                            if let Some(h) = git::Git::get_short_hash(&repo_dir) {
                                let url = format!(
                                    ...
                                    config.git.org, project_name, stu, h
                                );
                                let header =
                                    format!("Test results for repo as of this commit: {}\n\n", url);
                                if !rr
                                    .comment
                                    .starts_with("Test results for repo as of this commit:")
                                {
                ...
                if let Err(e) = runner.write_class_json(&only_results, suffix_opt.as_deref()) {
                    print_red(&format!("{}\n", e));
                    std::process::exit(3);
                }
            }
        ...
        Commands::Exec {
            ...
            let suffix_opt: Option<String> = if *by_date {
                ...
                match d.select() {
                    Some(sel) => Some(sel.suffix.clone()),
                    None => {
                        return;
                    }
                }
            ...
            let mut repos = vec![];
            for s in list {
                repos.push(testcases::Repo::student(
                    project_name.clone(),
                    s,
                    runner.project_subdir(),
                    suffix_opt.clone(),
            ...
            if threads == 1 {
                for r in &repos {
                    ...
                    let opts = ExecOptions {
                        cwd: Some(r.local_path.to_string_lossy().to_string()),
                        ..Default::default()
                    };
                    let cmdline = vec![
                        String::from("/bin/sh"),
        ...
        Commands::Clone {
            ...
            let (suffix_opt, date_opt) = if *by_date {
                let runner =
                    TestRunner::new(&config.test, false, false, false, project_name.clone());
                let d = match dates::Dates::from_tests_path(&runner.tests_path, &project_name) {
                    Ok(d) => d,
                    Err(e) => {
                        print_red(&format!("{}\n", e));
        ...
        Commands::View {
            ...
            let json_path = if *by_date && file.is_none() {
                ...
                file.as_deref()
                    .map(|s| s.to_string())
                    .unwrap_or_else(|| format!("{}.json", project_name))
            };
...
",
            ...
            let results: Vec<testcases::RepoResult> = match serde_json::from_str(&data) {
                Ok(v) => v,
                Err(e) => {
                    print_red(&format!(
                        "Failed to parse {}: {}
",
            ...
            if let Some(stem) = std::path::Path::new(&json_path)
                .file_stem()
                .and_then(|s| s.to_str())
            {
            ...
            for rr in &results {
                ...
                let repo =
                    Repo::student(project_name.clone(), s, subdir.clone(), suffix_opt.clone());
            ...
            for (rr, lbl) in results.iter().zip(labels.iter()) {
                ...
                if rr.results.is_empty() {
                    ...
                    println!(
                        "{}",
                        crate::testcases::TestRunner::make_earned_avail_static(&rr.results)
                    );
                }
            }
        ...
        Commands::Upload {
            ...
            let project_name = project.clone().unwrap_or_else(util::project_from_cwd);
            if let Err(e) = canvas::upload_class(
                config.canvas.clone(),
                config.canvas_mapper.clone(),
                &project_name,
                file.as_deref(),
                *verbose,
                *by_date,
        ...
        Commands::Rollup {
            ...
            if let Err(e) = rollup::rollup(&project_name, &d.items) {
                print_red(&format!("{}\n", e));
                std::process::exit(64);
            }
        }
