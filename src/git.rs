...
impl Git {
    ...
    fn get_commit_before(local: &PathBuf, branch: &str, date: &str) -> anyhow::Result<String> {
        ...
        let s = Self::run_capture(
            &["git", "rev-list", "-n", "1", "--first-parent", &arg, branch],
            Some(local),
        )?;
        let hash = s.lines().next().unwrap_or("").trim().to_string();
        if hash.is_empty() {
            anyhow::bail!("No commits in date range");
        }
        Ok(hash)
    }
    ...
    pub fn clone_repo(&self, project: &str, repo: &Repo, date: Option<&str>, verbose: bool) {
        ...
        let ok = if verbose {
            Self::run_ok(
                &["git", "clone", &url, &repo.local_path.to_string_lossy()],
                None,
            )
        } else {
            Self::run_ok_quiet(
        ...
        .unwrap_or(false);
        if !ok {
            print_red("No remote repo or clone failed\n");
            return;
        }
        ...
        if let Some(d) = date {
            ...
            if let Ok(branch) = Self::get_default_branch(&repo.local_path) {
                match Self::get_commit_before(&repo.local_path, &branch, d) {
                    Ok(hash) => {
                        let _ = if verbose {
                    ...
                    Err(_) => {
                        ...
                        let _ = Self::run_ok_quiet(
                            &["rm", "-rf", &repo.local_path.to_string_lossy()],
                            None,
                        );
                    }
                }
...
mod tests {
    ...
    fn remote_url_formats() {
        let g_ssh = Git::new(GitCfg {
            org: "orgx".into(),
            credentials: "ssh".into(),
        });
        let g_https = Git::new(GitCfg {
            org: "orgx".into(),
            credentials: "https".into(),
        });
        assert_eq!(
            g_ssh.remote_url("projx-alice"),
            "git@github.com:orgx/projx-alice.git"
        );
        assert_eq!(
            g_https.remote_url("projx-alice"),
            "https://github.com/orgx/projx-alice"
        );
    }
