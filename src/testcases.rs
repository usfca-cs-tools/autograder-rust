...
use crate::util::{
    expand_tilde, format_pass_fail, normalize_lines, print_green, print_red, print_yellow,
};
...
pub struct ProjectCfg {
    ...
    pub build: String,
    #[serde(default)]
    pub strip_output: Option<String>,
    #[serde(default)]
...
impl Repo {
    ...
    pub fn student(
        ...
        Repo {
            ...
            display_label: p.to_string_lossy().to_string(),
            local_path: p,
        }
    }
}
...
impl TestRunner {
    ...
    pub fn project_subdir(&self) -> Option<String> {
    ...
    fn load_testcases(&mut self) -> anyhow::Result<()> {
        ...
        let content = fs::read_to_string(&path)
            .map_err(|e| anyhow::anyhow!("File not found: {} ({})", path.display(), e))?;
        let doc: ProjectToml = toml::from_str(&content)
            .map_err(|e| anyhow::anyhow!("Failed to parse {}: {}", path.display(), e))?;
        if let Some(pcfg) = doc.project {
            self.project_cfg = pcfg;
        }
        ...
        match self.project_cfg.build.as_str() {
            ...
            "make" => {
                if !repo.local_path.exists() {
                    return Some(format!("Repo not found: {}", repo.local_path.display()));
                }
                let mfu = repo.local_path.join("Makefile");
                let mfl = repo.local_path.join("makefile");
                if !mfu.is_file() && !mfl.is_file() {
                    return Some(format!("Makefile not found: {}", mfu.display()));
                }
                let cmd = vec![
                    "make".to_string(),
                    "-C".to_string(),
    ...
    fn run_one_test(&self, repo: &Repo, tc: &TestCaseCfg) -> TcResult {
        ...
        let opts = ExecOptions {
            cwd: Some(repo.local_path.to_string_lossy().to_string()),
            timeout,
        ...
        for i in tc.input.iter() {
            cmdline.push(self.interpolate(i, &tc.name));
        }
        let actual_res = if tc.output == "stdout" {
            match crate::cmd::exec_capture_with_status(&cmdline, &opts) {
                ...
                Ok((out, false, code)) => {
                    ...
                    let enoexec_like = lower.contains("exec format error")
                        || matches!(code, Some(126) | Some(193));
                    if enoexec_like {
                        Err(crate::cmd::ExecError::Io(
            ...
            fs::read_to_string(&f).map_err(crate::cmd::ExecError::Io)
        };

        match actual_res {
            Ok(mut actual) => {
                if let Some(strip) = &self.project_cfg.strip_output {
                    actual = actual.replace(strip, "");
                }
                if self.match_expected(tc, &actual) {
                    result.score = tc.rubric;
                }
                self.print_verbose(tc, &cmdline, &actual);
            }
            Err(e) => {
                let msg = match e {
                    crate::cmd::ExecError::Timeout(_) => {
                        "Program timed out (infinite loop?)".to_string()
                    }
                    crate::cmd::ExecError::OutputLimit(_) => {
                        "Program produced too much output (infinite loop?)".to_string()
                    }
                    ...
                    crate::cmd::ExecError::Io(ref ioe)
    ...
    fn print_verbose(&self, tc: &TestCaseCfg, cmdline: &[String], actual: &str) {
        ...
        if self.very_verbose {
            println!(
                ...
                tc.name,
                cmd_display,
                self.interpolate(&tc.expected, &tc.name)
            );
    ...
    fn make_comment(&self, repo_result: &RepoResult) -> String {
        ...
        if let Some(be) = &repo_result.build_err {
            prefix.push_str(be);
            prefix.push(' ');
        }
    ...
    pub fn test_repo(
        ...
        let iter = self
            .testcases
            .iter()
            .filter(|tc| only_name.map(|n| n == tc.name).unwrap_or(true));
        for tc in iter {
            results.push(self.run_one_test(repo, tc));
        }
        let score = results.iter().map(|r| r.score).sum();
        let mut repo_result = RepoResult {
            comment: String::new(),
            results,
            score,
            student: repo.student.clone(),
    ...
    pub fn total_rubric(&self) -> i64 {
        self.testcases.iter().map(|tc| tc.rubric).sum()
    }
    ...
    pub fn print_histogram(&self, class_results: &[RepoResult]) {
        ...
        for r in class_results {
            *freqs.entry(r.score).or_default() += 1;
        }
        ...
        for (score, freq) in items {
            let pct = (freq as f64) / (class_results.len() as f64) * 100.0;
            println!("{}/{}: {}  ({:.1}%)", score, avail, freq, pct);
        }
    }

    pub fn write_class_json(
        &self,
        class_results: &[RepoResult],
        suffix: Option<&str>,
    ) -> anyhow::Result<()> {
        let fname = if let Some(s) = suffix {
            format!("{}-{}.json", self.project, s)
        } else {
            format!("{}.json", self.project)
        };
