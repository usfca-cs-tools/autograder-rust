...
impl Github {
    ...
    fn get_first_artifact(&self, student: &str) -> anyhow::Result<Option<Artifact>> {
        ...
        if !res.status().is_success() {
            print_yellow(&format!(
                "Accessing artifacts for {} returned {}\n",
                student,
                res.status()
            ));
            return Ok(None);
    ...
    fn get_action_run_summary_url(&self, student: &str, run_id: u64) -> anyhow::Result<String> {
        ...
        if !res.status().is_success() {
            ...
            return Ok(format!(
                ...
                self.org, self.project, student, run_id
            ));
        }
        let jobs: JobsList = res.json()?;
        let job_id = jobs.jobs.first().map(|j| j.id).unwrap_or(0);
        Ok(format!(
    ...
    pub fn get_action_results(&self, student: &str) -> RepoResult {
        ...
        match self.get_first_artifact(student) {
            Ok(Some(artifact)) => {
                ...
                let grade = self.download_artifact_grade(&artifact).unwrap_or_else(|e| {
                    print_yellow(&format!("Analyzing artifact: {}\n", e));
                    0.0
                });
                let link = self
                    .get_action_run_summary_url(student, run_id)
